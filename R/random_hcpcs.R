...
random_hcpcs <- function(n = 10, lng = c(1:5)) {
  ...
  sf_convert(
    sf_c(
      if (any(lng == 1)) sample_(x = p, size = sample.int(5, 1)) else NULL,
      if (any(lng == 2)) sf_sub(sample_(x = h, size = n), stop = 2) else NULL,
      if (any(lng == 3)) sf_sub(sample_(x = h, size = n), stop = 3) else NULL,
      if (any(lng == 4)) sf_sub(sample_(x = h, size = n), stop = 4) else NULL,
      if (any(lng == 5)) sf_sub(sample_(x = h, size = n), stop = 5) else NULL
      )
    )

}

#' Generate Random HCPCS Codes Vector (Old)
#'
#' @param n `<integer>` number of codes; default is `10`
#'
#' @returns `<character>` vector
